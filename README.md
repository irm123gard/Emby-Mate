...
- 发布快照：`worker-v2.4.9.js`
- 默认 KV 绑定名：`ENI_KV`
- 部署配置：`wrangler.jsonc`
- 当前 GitHub 仓库仅保留发布与部署所需文件

## 快速入口

- 更新日志：[`CHANGELOG.md`](./CHANGELOG.md)
- 构建产物：[`dist/worker.js`](./dist/worker.js)
- 发布快照：[`worker-v2.4.9.js`](./worker-v2.4.9.js)
- 部署配置：[`wrangler.jsonc`](./wrangler.jsonc)

## v2.4.9 发布重点

- 主线继续优先打磨播放稳定性：首播 `followed/html 4xx` 会更保守地回退，后台新增可选代理诊断头，更适合直接从 HAR / Network 里定位异常链路。
- 播放与运行时性能继续收紧：新增短时 `head window / jump window`、早期 jump 的窄范围预算放宽、节点索引与运行时缓存分步清理，减少无效回源和热点状态膨胀。
- 管理后台体感更轻：节点列表改为增量复用渲染，搜索、`tcping`、客户端 RTT、节点活跃和 CF 指标自动刷新都加入了去重、短窗口复用和隐藏页跳过策略。
- 发布仓库继续聚焦单文件部署，保留 `dist/worker.js` 与版本快照两条交付入口。
...
1. 在 Cloudflare 创建 Worker，用 `worker-v2.4.9.js` 走 Dashboard 粘贴部署，或用 `dist/worker.js` 走 Wrangler 工程化部署。
...
- `worker-v2.4.9.js`
  - 版本快照
  - 适合 Cloudflare Dashboard 直接粘贴部署
  - 内容来自 `npm run release:snapshot`，不建议手工直接维护

如果你准备接入 GitHub、Wrangler 或后续 CI/CD，请始终以 `dist/worker.js` 为准。
...
当前 GitHub 仓库以发布和部署为主，只保留可直接部署到 Cloudflare Workers 的文件。
...
2. 创建 Worker，并粘贴 `worker-v2.4.9.js` 全部内容。
...
- 如果只是你自己部署，不需要一键分享，继续用 Cloudflare Dashboard 直接粘贴 `worker-v2.4.9.js` 会更省事。
